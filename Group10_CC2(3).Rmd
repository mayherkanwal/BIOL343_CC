...
----------------|----------------------------------|------
population_type |asexual or sexual                 | 27
ovary_width     |width of ovary                    | 29
ovary_height    |height of ovary                   | 31
flower_number   |number of flowers per population  | 31
...
```{r import-clean}
cc1_data <- read.csv("./data/A1_Data_Excel(2).csv")
...
area_means <- aggregate(ovary_area~population_type,data=cc1_data_mod,FUN = mean)
...
ggplot(cc1_data_mod, aes(x=ovary_area))+geom_histogram(binwidth=1)+facet_wrap(~population_type)+labs(x="ovary area(mm^2)",y="count",title="Ovary Area by population type")+theme_classic()
```
...
![Solanum pimpinellifolium](./images/Solanum_pimpinellifolium.png)

Source: [Solanum pimpinellifolium](https://onq.queensu.ca/d2l/le/dropbox/1208667/492536/DownloadAttachment?fid=54136532)
